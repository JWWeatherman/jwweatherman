...
const GITHUB_CLIENT_ID = process.env.GITHUB_CLIENT_ID || keys.github.clientId
const GITHUB_SECRET = process.env.GITHUB_SECRET || keys.github.secret
...
function getMarkdown ({config, done, res}) {
  axios.get(config.REPO_URL + '?client_id=' + GITHUB_CLIENT_ID + '&client_secret=' + GITHUB_SECRET)
    .then(response => {
...
function configGen (res) {
  axios.get(CONFIGS_URL + '?client_id=' + GITHUB_CLIENT_ID + '&client_secret=' + GITHUB_SECRET)
    .then(response => {
      const b64 = base64.Base64
      const blob = response.data.content
